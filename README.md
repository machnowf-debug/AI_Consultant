...
| Datum | Änderung |
|---|---|
| 2026-07-28 | **Fehler behoben, der nur live aufgetreten wäre:** Die Content-Security-Policy erlaubte kein Inline-Script, das Script im `<head>` beider Startseiten wäre also auf Netlify stillschweigend blockiert worden. Der zugehörige `sha256`-Hash steht jetzt in `netlify.toml` und `_headers`. Wird das Inline-Script geändert, muss der Hash neu berechnet werden |
